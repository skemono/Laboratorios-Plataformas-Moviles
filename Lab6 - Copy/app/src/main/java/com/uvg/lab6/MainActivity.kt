...
fun handleLabStates(modifier: Modifier){
    ...
    var historial = remember { mutableStateListOf<Int>() }
    ...
    fun onContadorChange(dir: String) {
        ...
        historial.add(contador)
        maximo = historial.max()
        minimo = historial.min()
        cambios++
...
fun Lab6(
    ...
    minimo: Int,
    cambios: Int,
    historial: SnapshotStateList<Int>
) {
    Column(
        modifier = Modifier
            .fillMaxSize()
        ...
        LazyVerticalGrid(
            ...
            items(historial.size) { index ->
                Button(
                    ...
                    Text("${historial[index]}", color = MountbattenPink)
                }
            }
        }
